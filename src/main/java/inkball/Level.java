...
public class Level {
  ...
  private final double[] scoreIncrease = new double[5];
  private final double[] scoreDecrease = new double[5];

  private static final double[] generalScoreIncrease = new double[5];
  private static final double[] generalScoreDecrease = new double[5];
  ...
  /**
   * Draws times up message for when level timer ran out
  ...
  void drawBalls(PApplet window) {
    ...
    for (Iterator<Ball> it = this.balls.iterator(); it.hasNext(); ) {
      ...
      if (holeLoc != null) {
        ...
        if (b.color == 0 || hole.getColor() == 0) {
          this.addScore((float) this.scoreIncrease[b.color]);
        }

        else if (b.color == hole.getColor()) {
          this.addScore((float) this.scoreIncrease[b.color]);
        }
