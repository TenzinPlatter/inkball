...
public class App extends PApplet {
  ...
  void loadSprites() {
    ...
    String[] spriteTypes = { "ball", "hole", "wall", "crackedWall" };
    for (int i = 0; i <= 4; i++) {
      for (String spriteType : spriteTypes) {
        PImage result = loadImageFromPath(spriteType + i + ".png");
        System.out.println(spriteType + i + ".png");
        if (result != null) {
          System.out.println(spriteType + i + ".png" + " worked");
          sprites.put(spriteType + i, result);
        }
      }
    }
  ...
  public void keyPressed(KeyEvent event){
    ...
    // stop functionality if level over
    if (this.currentLevel.timerEmpty() || this.currentLevel.inEndAnim) {
      return;
    }
